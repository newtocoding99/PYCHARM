...
class MainWindow(QWidget):
    ...
    def __init__(self, set_numbers_to_rows, master=None):
        ...
        self.button_1 = QPushButton('Next Slice', self)
        main_layout.addWidget(self.button_1)
        self.buttons.append(self.button_1)
        self.button_1.clicked.connect(self.next_image)

        self.button_2 = QPushButton('Clear', self)
        main_layout.addWidget(self.button_2)
        self.buttons.append(self.button_2)
        self.button_2.clicked.connect(self.clear_rect)

        self.button_1 = QPushButton('   Draw box/Click Box  ', self)
        main_layout.addWidget(self.button_1)
        self.buttons.append(self.button_1)
        self.button_1.clicked.connect(self._toggle_drawing)

        self.button_2 = QPushButton('    Next Set  ', self)
        main_layout.addWidget(self.button_2)
        self.buttons.append(self.button_2)
        self.button_2.clicked.connect(self.get_next_image_set())
        ...
        if predictions_bbox is not None and predictions_bbox != '':
            for item in predictions_bbox.split(":"):
                ...
                if len(coordinates) >= 4:
                    ...
                    left_or_right_line = ((l - 4) <= x <= (l + 4) or (r - 4) <= x <= (r + 4)) and (t - 4) <= y <= \
                                (b + 4)
                    bottom_or_top_line = ((t - 4) <= y <= (t + 4) or (b - 4) <= y <= (b + 4)) and (l - 4) <= x <= (
                                r + 4)
                    if left_or_right_line or bottom_or_top_line:
                        self.canvas.create_rectangle(l, t, r, b, tag="box", outline="magenta", )
                        self.canvas.pack(expand=1)
...
if __name__ == "__main__":
    ...
    app = QApplication(set_number_to_rows)
